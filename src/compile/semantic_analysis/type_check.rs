...
impl ProgramIr {
    pub fn ty_check(mut self) -> TyCheckResult<ProgramIr> {
        ...
        let func_name_list: Vec<String> =
            self.func_list.keys().map(|x| x.clone()).clone().collect();
        for func_name in func_name_list {
            let func_ir = self.func_list.remove(&func_name).unwrap().ty_check(
                &mut self.func_list,
                &self.ex_dec_func_list,
                Type::Unknown,
            )?;
...
impl FuncIr {
    fn ty_check(
        ...
        self.body = self.body.ty_check(
            func_list,
            ex_func_list,
            &mut self.ty.param_types,
            self.ty.ret_type,
        )?;
...
impl ExprIr {
    fn ty_check(
        ...
        Ok(match self {
            ExprIr::NumIr(x) => ExprIr::NumIr(x.ty_check()),
            ExprIr::CallIr(x) => ExprIr::CallIr(Box::new(x.ty_check(
                func_list,
                ex_func_list,
                params,
                expect_ty,
            ...
            ExprIr::VariableIr(x) => ExprIr::VariableIr(x.ty_check(params, expect_ty)?),
...
impl CallIr {
    fn ty_check(
        ...
        self.func = self
            .func
            .ty_check(func_list, ex_func_list, params, expect_func_ty)?;
...
impl OpIr {
    fn ty_check(
        ...
        self.l_expr = self
            .l_expr
            .ty_check(func_list, ex_func_list, params, Type::Int32)?;
        self.r_expr = self
            .r_expr
            .ty_check(func_list, ex_func_list, params, Type::Int32)?;
        self.ty = Type::Int32;
...
impl GlobalVariableIr {
    fn ty_check(
        ...
        ex_func_list: &ExFuncList,
        expect_ty: Type,
    ) -> TyCheckResult<GlobalVariableIr> {
        let func_ty = match func_list.remove(&self.id) {
            Some(x) => {
            ...
            _ => match ex_func_list.get(&self.id) {
