...
impl VariableTable {
    ...
    pub fn get_variable_ir(&self, var: VariableAST, ty_sub: &mut TypeSubstitute) -> Option<ExprMir> {
        let a = self.local_var_names[self.nest_level - 1]
            .iter().rev().enumerate()
            .find(|(_, name)| *name == &var.id)
            .map(|(id, _)| id);
        match a {
            Some(id) => Some(ExprMir::create_variable_mir(id, var.pos, ty_sub.ty_env.fresh_type_id())),
            _ => {
                if self.global_var_names.contains_key(&var.id) {
                    Some(ExprMir::create_global_variable_mir(var.id, var.pos, ty_sub.ty_env.fresh_type_id()))
                } else {
                    None
                }
            }
